...
def _write_database_raw_snapshot(
    dataframe: pd.DataFrame,
    output_file: Path,
) -> None:
...
def _write_run_report(
    ...
    lines = [
        ...
        (
            "- CSV records: "
            f"**{metrics.source_records.get('CSV', 0)}**"
        ),
        (
            "- API records: "
            f"**{metrics.source_records.get('API', 0)}**"
        ),
        (
            "- Database records: "
            f"**{metrics.source_records.get('DATABASE', 0)}**"
        ),
        (
            "- Source records total: "
            f"**{metrics.source_records_total}**"
        ),
        (
            "- Source rejected records: "
            f"**{metrics.source_rejected_records}**"
        ),
        (
            "- Integration rejected records: "
            f"**{metrics.integration_rejected_records}**"
        ),
        (
            "- Integrated records: "
            f"**{metrics.integrated_records}**"
        ),
        (
            "- Valid final records: "
            f"**{metrics.valid_final_records}**"
        ),
        (
            "- Rejected records: "
            f"**{metrics.rejected_records}**"
        ),
        (
            "- Duplicate records: "
        ...
        (
            ...
            f"**{metrics.missing_values_handled}**"
        ),
        (
            "- Cross-source mismatches: "
            f"**{metrics.cross_source_mismatches}**"
        ),
        (
            "- Processing time: "
            f"**{metrics.duration_seconds:.4f} seconds**"
        ),
        (
            "- Cache hits: "
    ...
    config.output.run_report.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
...
def _config_fingerprint(config: AppConfig) -> str:
    payload = {
        ...
        "minimum_attendance_good": (
            config.processing.minimum_attendance_good
        ),
    }
...
def _reuse_previous_run(
    ...
    metrics.source_records = previous_metrics.get(
        "source_records",
        {},
    )

    metrics.source_records_total = previous_metrics.get(
        "source_records_total",
        sum(metrics.source_records.values()),
    )

    metrics.source_rejected_records = previous_metrics.get(
        "source_rejected_records",
        sum(
    ...
    metrics.integration_rejected_records = previous_metrics.get(
        "integration_rejected_records",
        previous_metrics.get(
    ...
    metrics.integrated_records = previous_metrics.get(
        "integrated_records",
        0,
    )

    metrics.valid_records = previous_metrics.get(
        "valid_records",
        0,
    )

    metrics.valid_final_records = previous_metrics.get(
        "valid_final_records",
        previous_metrics.get(
            "valid_records",
            0,
    ...
    metrics.rejected_records = previous_metrics.get(
        "rejected_records",
        0,
    )

    metrics.duplicate_records = previous_metrics.get(
        "duplicate_records",
        0,
    )

    metrics.missing_values_handled = previous_metrics.get(
        "missing_values_handled",
        0,
    )

    metrics.invalid_records = previous_metrics.get(
        "invalid_records",
        0,
    )

    metrics.cross_source_mismatches = previous_metrics.get(
        "cross_source_mismatches",
        0,
    )

    metrics.rejected_by_source = previous_metrics.get(
        "rejected_by_source",
        {},
    )

    metrics.rejection_reasons = previous_metrics.get(
        "rejection_reasons",
        {},
    )

    metrics.outputs = previous_metrics.get(
        "outputs",
        {},
    )

    metrics.total_records = previous_metrics.get(
        "total_records",
        metrics.source_records_total,
    )
...
def run_pipeline(
    ...
    if selected_mode not in {
        ...
        raise ValueError(
            "mode must be either 'full' or 'incremental'"
        )
    ...
    logger.info(
        "Pipeline started | run_id=%s | mode=%s",
        run_id,
        selected_mode,
    )

    csv_hash = sha256_file(
        config.sources.csv_path
    )

    database_hash = sha256_file(
        config.sources.database_path
    )

    api_endpoint_hash = _api_url_fingerprint(
    ...
    previous_source_hashes = previous_manifest.get(
        "source_hashes",
        {},
    )

    # The remote API must still be contacted in incremental mode
    # because its payload can change while its URL stays the same.
    logger.info(
        "REST API extraction started"
    )
    ...
    all_sources_unchanged = bool(
        ...
        and previous_source_hashes == current_hashes
    )

    if (
        selected_mode == "incremental"
        and previous_manifest.get(
            "config_hash"
        ) == config_hash
        ...
        logger.info(
            ...
            "reusing previous pipeline outputs."
        )
    ...
    # CSV
    # ------------------------------------------------------------------
    previous_csv_hash = previous_source_hashes.get(
        "csv"
    )
    ...
    if csv_unchanged:
        ...
        metrics.source_records["CSV"] = (
            len(csv_valid)
            + len(csv_rejected)
        )

        logger.info(
            "CSV source unchanged; reused validated cache."
        )
        ...
        metrics.source_records["CSV"] = len(
            csv_raw
        )
        ...
        metrics.duplicate_records += (
            csv_stats["exact_duplicates"]
            + csv_stats["duplicate_student_ids"]
        )

        metrics.missing_values_handled += (
            csv_stats[
                "missing_values_handled"
            ]
        )
    ...
    # REST API
    # ------------------------------------------------------------------
    previous_api_hash = previous_source_hashes.get(
        "api_payload"
    )
    ...
    if api_unchanged:
        ...
        metrics.source_records["API"] = (
            len(api_valid)
            + len(api_rejected)
        )

        logger.info(
            "REST API payload unchanged; "
            "reused validated cache."
        )

    else:
        metrics.source_records["API"] = len(
            api_raw
        )

        _write_api_raw_snapshot(
            api_raw,
            config.output.api_raw_snapshot,
        )
        ...
        (
            ...
            api_rejected,
            api_stats,
        ) = clean_and_validate_api(
            api_raw
        )

        metrics.duplicate_records += (
            api_stats["exact_duplicates"]
            + api_stats["duplicate_student_ids"]
        )
    ...
    # SQLite
    # ------------------------------------------------------------------
    previous_db_hash = previous_source_hashes.get(
        "database"
    )

    db_cache = (
        _read_cache("database")
        if selected_mode == "incremental"
        else None
    )
    ...
    if db_unchanged:
        ...
        metrics.source_records["DATABASE"] = (
            len(database_valid)
            + len(database_rejected)
        )
        ...
        logger.info(
            "SQLite extraction started"
        )

        database_raw = extract_enrollments(
            config.sources.database_path
        )

        metrics.source_records["DATABASE"] = len(
            database_raw
        )
    ...
    # ------------------------------------------------------------------
    metrics.source_records_total = sum(
        metrics.source_records.values()
    )

    metrics.source_rejected_records = (
        len(csv_rejected)
        + len(api_rejected)
        + len(database_rejected)
    )
    ...
    metrics.integration_rejected_records = len(
        integration_rejected
    )

    metrics.cross_source_mismatches = len(
        integration_rejected
    )
    ...
    if integrated.empty:
        raise ValueError(
            "Integration produced no compatible records."
        )
    ...
    metrics.valid_final_records = len(
        integrated
    )
    ...
    student_ml_dataset = build_student_ml_dataset(
        integrated
    )
    ...
    metrics.outputs = {
        "final_dataset": str(
            config.output.final_dataset.relative_to(
                BASE_DIR
            )
        ),
        "student_ml_dataset": str(
            config.output.student_ml_dataset.relative_to(
                BASE_DIR
            )
        ),
        "rejected_records": str(
            config.output.rejected_records.relative_to(
                BASE_DIR
            )
        ),
        "metrics_report": str(
            config.output.metrics_report.relative_to(
                BASE_DIR
            )
        ),
        "run_report": str(
            config.output.run_report.relative_to(
                BASE_DIR
            )
        ),
        "log_file": str(
            config.output.log_file.relative_to(
                BASE_DIR
            )
        ),
        "api_raw_snapshot": str(
            config.output.api_raw_snapshot.relative_to(
                BASE_DIR
            )
        ),
        "database_raw_snapshot": str(
            config.output.database_raw_snapshot.relative_to(
                BASE_DIR
            )
        ),
    }
    ...
    logger.info(
        "Final dataset created: %s",
        config.output.final_dataset,
    )

    logger.info(
        "Rejected records written: %s",
        config.output.rejected_records,
    )

    logger.info(
        "Pipeline completed successfully | run_id=%s",
        run_id,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Processing mode. "
            "Defaults to config.json setting."
        ),
    )
